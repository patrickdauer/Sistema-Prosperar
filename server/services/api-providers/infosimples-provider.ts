...
import { ApiProvider, DasResult } from './types';

export class InfoSimplesProvider implements ApiProvider {
  private token: string;
  private baseUrl: string = 'https://api.infosimples.com/api/v2/consultas/receita-federal/simples-das';
  ...
  async generateDas(cnpj: string, periodo: string): Promise<DasResult> {
    try {
      ...
      if (data.code !== 200) {
        throw new Error(data.code_message || 'Erro na API InfoSimples');
      }
      ...
      const dasData = data.data[0];
      const periodoData = dasData.periodos[periodo];

      if (!periodoData) {
        throw new Error(`Período ${periodo} não encontrado`);
      }

      return {
        success: true,
        data: {
          urlDas: periodoData.url_das,
          dataVencimento: periodoData.data_vencimento,
          valorTotal: periodoData.valor_total_das,
          situacao: periodoData.situacao,
          razaoSocial: dasData.razao_social,
          periodo: periodoData.periodo,
          numeroApuracao: periodoData.numero_apuracao,
          numeroDas: periodoData.numero_das,
          codigoBarras: periodoData.codigo_barras_das
        }
      };
    } catch (error) {
      return {
  ...
  async testConnection(): Promise<boolean> {
    try {
      // Teste simples com CNPJ fictício
      const result = await this.generateDas('00000000000000', '202501');
